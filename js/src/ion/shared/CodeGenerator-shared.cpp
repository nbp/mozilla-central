...
bool
CodeGeneratorShared::encode(LResumePoint *rp)
{
    if (rp->offset() != INVALID_RECOVER_OFFSET)
        return true;
            ...
            if (GetIonContext()->cx) {
                ...
                if (JSOp(*bailPC) == JSOP_FUNCALL) {
                    ...
                    JS_ASSERT(stackDepth - exprStack <= 1);
                           ...
                           !IsGetterPC(bailPC) && !IsSetterPC(bailPC))
                {
                    ...
                    // With accessors, we have different stack depths depending
                    // on whether or not we inlined the accessor, as the inlined
                    // stack contains a callee function that should never have
                    // been there and we might just be capturing an uneventful
                    // property site, in which case there won't have been any
                    // violence.
...
CodeGeneratorShared::callVM(const VMFunction &fun, LInstruction *ins, const Register *dynStack)
{
    ...
    if (ins->mirRaw()) {
        ...
        JS_ASSERT_IF(mir->isEffectful(), mir->block()->numSuccessors() == 0 || mir->resumePoint());
    }
#endif
